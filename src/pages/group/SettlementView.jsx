...
export default function SettlementView() {
  ...
  const { id } = useParams()
  const { groups, getSettlement, expenses, recordPayment } = useGroup()
        ...
        {transactions.map((t, i) => (
          <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px',borderRadius:'12px',background:'#F9FAFB',marginBottom:'8px'}}>
            ...
            <button
              ...
              style={{padding:'8px 16px',borderRadius:'10px',border:'none',fontSize:'13px',fontWeight:'600',cursor:'pointer',background:'#10B981',color:'#FFFFFF'}}
            >
              Mark Paid
            </button>
          </div>
