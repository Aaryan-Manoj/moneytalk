...
export default function CreateGroup() {
  ...
  const save = async () => {
    ...
    const groupId = await createGroup(name, memberUids, memberNames, [])
    for (const m of validMembers) {
      if (m.inviteId && groupId) {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#F9FAFB',padding:'32px',maxWidth:'560px',margin:'0 auto'}}>
      ...
      <button onClick={save} disabled={saving || !name} style={{background: name ? '#2563EB' : '#E5E7EB',color:'#FFFFFF',border:'none',borderRadius:'12px',padding:'14px',fontSize:'15px',fontWeight:'600',cursor: name ? 'pointer' : 'not-allowed',width:'100%'}}>
